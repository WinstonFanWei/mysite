...
jQuery.getJSON(GetQueryStringParams("config", "/static/myapp/misdiagnose_web/json/config.json"), function (data, textStatus, jqXHR) {
    config = data;
...
function setupGUI(config) {
    ...
    if (config.logo.link) logo = "<a href=\"" + config.logo.link + "\" target='_blank'>" + logo + "</a>";
    $("#maintitle").html(logo);
...
function configSigmaElements(config) {
    ...
    var a = [],
        b, x = 1;
    for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div>' + ((x++) == 2 ? "罕见病" : "常见病") + ' (' + sigInst.clusters[b].length + ' 个成员)') + '</a></div>';
    //a.sort();
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (2 >= a.length) this.results.html("<i>您需要输入至少3个字符</i>");
        else {
...
function nodeActive(a) {
    ...
    if (groupByDirection) {
        ...
        /* f.push("<h2>度数 (" + size + ")</h2>");
        (size > 0) ? f = f.concat(createList(mutual)) : f.push("No mutual links<br>"); */
        size = Object.size(incoming);
        f.push("<h2>可能被误诊为此病 (" + size + ")</h2>");
        (size > 0) ? f = f.concat(createList(incoming)) : f.push("没有找到结果<br>");
        size = Object.size(outgoing);
        f.push("<h2>此病可能被误诊为 (" + size + ")</h2>");
        (size > 0) ? f = f.concat(createList(outgoing)) : f.push("没有找到结果<br>");
    } else {
    ...
    if (f.attributes) {
        ...
        for (var attr in f.attributes) {
            ...
            if (attr != image_attribute) {
                ...
                h = '<span><strong>' + "是否为罕见病" + ':</strong> ' + aaa + '</span><br/>'
            };
